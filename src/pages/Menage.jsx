  ...
  if (mode === "mescollectes") return (
    <div style={{ padding: "16px", maxWidth: 440, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[
          { label: "Total", value: mesSignalements.length, icon: "📦", color: "#0f172a", bg: "linear-gradient(135deg, #f8fafc, #f1f5f9)", border: "#e2e8f0" },
          { label: "En cours", value: mesSignalements.filter(s => s.status === "en cours").length, icon: "⏳", color: "#d97706", bg: "linear-gradient(135deg, #fffbeb, #fef3c7)", border: "#fde68a" },
          { label: "Collectés", value: mesSignalements.filter(s => s.status === "collecté").length, icon: "✅", color: "#16a34a", bg: "linear-gradient(135deg, #f0fdf4, #dcfce7)", border: "#bbf7d0" },
        ].map(s => (
  ...
  return (
    <div style={{ padding: "16px", maxWidth: 440, margin: "0 auto" }}>
      ...
      {step === 0 && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 4 }}>
            {[
              { label: "Total", value: mesSignalements.length, icon: "📦", color: "#0f172a" },
              { label: "En cours", value: mesSignalements.filter(s => s.status === "en cours").length, icon: "⏳", color: "#d97706" },
              { label: "Collectés", value: mesSignalements.filter(s => s.status === "collecté").length, icon: "✅", color: "#16a34a" },
            ].map(s => (
      ...
      {step === 1 && (
        <div style={{ background: "white", borderRadius: 20, padding: 20, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            <div style={{ width: 32, height: 32, borderRadius: 10, background: "linear-gradient(135deg, #16a34a, #15803d)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>📍</div>
            <div>
          ...
          <button onClick={localiser} disabled={gpsLoading} style={{
            ...
            {gpsLoading ? "📡 Localisation en cours..." : gpsOk ? "✅ Position GPS obtenue !" : "📍 Ajouter ma localisation GPS"}
          </button>
      ...
      {step === 2 && (
        <div style={{ background: "white", borderRadius: 20, padding: 20, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            <div style={{ width: 32, height: 32, borderRadius: 10, background: "linear-gradient(135deg, #16a34a, #15803d)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🗑️</div>
            <div>
          ...
          {photoUploading && <p style={{ color: "#16a34a", fontSize: 12, textAlign: "center" }}>⏳ Upload en cours...</p>}
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: form.urgent ? "#ef4444" : "#92400e" }}>🔴 Marquer comme urgent</div>
          ...
          <div style={{ background: "#f0fdf4", borderRadius: 12, padding: "12px 14px", marginBottom: 16, border: "1px solid #bbf7d0" }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#16a34a" }}>📍 {form.commune} — {form.quartier}</div>
            {form.adresse && <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>{form.adresse}</div>}
            <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
              {gpsOk && <span style={{ fontSize: 10, background: "#dcfce7", color: "#16a34a", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>✅ GPS</span>}
              {form.photo && <span style={{ fontSize: 10, background: "#dcfce7", color: "#16a34a", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>📸 Photo</span>}
            </div>
          </div>
